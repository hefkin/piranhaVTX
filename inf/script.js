...
async function findMatchingTarget(
  ...
  function checkConfigObject(obj, path) {
    // Проверяем имеет ли объект нужные нам свойства
    if (
      obj &&
      ...
      for (const category of deviceCategories) {
        ...
        for (const configType in categoryData) {
          ...
          if (configData && typeof configData === "object") {
            ...
            // Проверяем вложенные объекты (для случаев как dual-core, nano и т.д.)
  ...
  try {
    ...
    // Проверяем layout каждого найденного таргета
...
function formatTargetURL(matchingTarget) {
  ...
  // Получаем базовые параметры из пути
  ...
  // Форматируем название таргета (заменяем пробелы на подчеркивание)
...
function getAppSegments(dataView) {
  ...
  if (magic === 0xffff) {
    if (dataView.getUint8(0x10000) === 0xe9) {
      ...
      throw new Error("Не найден заголовок, вероятно это не файл прошивки!")
    }
